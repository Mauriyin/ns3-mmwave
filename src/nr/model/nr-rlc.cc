...
void NrRlc::SetRnti(uint16_t rnti)
{
	NS_LOG_FUNCTION(this << (uint32_t)rnti);
	m_rnti = rnti;
}
void NrRlc::SetLcId(uint8_t lcId)
{
	NS_LOG_FUNCTION(this << (uint32_t)lcId);
	m_lcid = lcId;
}
void NrRlc::SetNrRlcSapUser(NrRlcSapUser *s)
{
...
void NrRlc::SetNrMacSapProvider(NrMacSapProvider *s)
{
	NS_LOG_FUNCTION(this << s);
	m_macSapProvider = s;
}
...
void NrRlc::DoNotifyHarqDeliveryFailure(uint8_t harqId)
{
	NS_LOG_FUNCTION(this);
}
} // namespace ns3
